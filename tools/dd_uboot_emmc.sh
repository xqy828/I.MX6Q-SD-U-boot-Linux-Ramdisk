...
if [ -z "$1" ];then
    echo "Usage: arg1=image arg2=/dev/mmcblk3boot0"
    exit
fi

#echo "erase u-boot sector in SD card, memory size =512*2048=1MB"
#dd if=/dev/zero of=$2 bs=512 count=2048 conv=fsync
#sync
echo "access boot partition 1"
echo 0 > /sys/block/$2/force_ro
echo "access linux boot partition 1"
echo 0 > /sys/block/mmcblk3/force_ro

echo "write $1 to $2:"
# Write u-boot.imx to SD card
# uboot.imx from 1KB-1MB(skip 512*2=1KB in eMMC)
dd if=$1 of=$2 bs=512 seek=2 conv=fsync
sync
echo "re-enable read-only access"
echo 1 > /sys/block/$2/force_ro
